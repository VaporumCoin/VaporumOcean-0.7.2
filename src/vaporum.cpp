...
int32_t vaporum_parsestatefile(struct vaporum_state *sp,FILE *fp,char *symbol,char *dest)
{
    int32_t func;
    ...
    catch(const vaporum::parse_error& pe)
    {
        ...
        LogPrintf("vaporumstate file is invalid. Vaporumd will be stopped. Please remove vaporumstate and vaporumstate.ind files and start the daemon");
        // std::cerr << std::endl << " Error in vaporumstate file: unknown event " << (char)func << ", exiting. Please remove vaporumstate and vaporumstate.ind files and start the daemon" << std::endl << std::endl;
        uiInterface.ThreadSafeMessageBox(_("Please remove vaporumstate and vaporumstate.ind files and restart"), "", CClientUIInterface::MSG_ERROR);
        StartShutdown();            
        func = -1;
    }
    return func;
...
int32_t vaporum_parsestatefiledata(struct vaporum_state *sp,uint8_t *filedata,long *fposp,long datalen,char *symbol,char *dest)
{
    int32_t func = -1;
        ...
        uint8_t *opretbuf,uint16_t opretlen,uint16_t vout,uint256 MoM,int32_t MoMdepth)
{
    ...
    if ( fp == 0 )
    {
        vaporum_statefname(fname,chainName.symbol().c_str(),(char *)"vaporumstate");
...
{
    static int32_t last_rewind; int32_t rewindtarget; CBlockIndex *pindex; struct vaporum_state *sp; char symbol[KOMODO_ASSETCHAIN_MAXLEN],dest[KOMODO_ASSETCHAIN_MAXLEN];
    if ( (sp= vaporum_stateptr(symbol,dest)) == 0 )
        return(0);
        ...
        uint64_t signedmask,uint32_t timestamp)
{
    ...
    int32_t opretlen,nid,offset,k,MoMdepth,matched,len = 0; uint256 MoM,srchash,desttxid; uint8_t crypto777[33]; struct vaporum_state *sp; char symbol[KOMODO_ASSETCHAIN_MAXLEN],dest[KOMODO_ASSETCHAIN_MAXLEN];
    if ( (sp= vaporum_stateptr(symbol,dest)) == 0 )
        return(-1);
    ...
    if ( scriptbuf[len++] == 0x6a )
    {
        ...
        else
        {
            if ( scriptbuf[len] == 'K' )
            {
                ...
                vaporum_stateupdate(height,0,0,0,txhash,0,0,0,0,value,&scriptbuf[len],opretlen,j,zero,0);
                return(-1);
            }
                ...
                if ( matched != 0 && *notarizedheightp > sp->LastNotarizedHeight() && *notarizedheightp < height )
                {
                    ...
                    if ( chainName.isKMD() )
                    {
                        ...
                        if ( opretlen > len && scriptbuf[len] == 'A' )
                        {
                            ...
                            vaporum_stateupdate(height,0,0,0,txhash,0,0,0,0,value,&scriptbuf[len],opretlen-len+4+3+(scriptbuf[1] == 0x4d),j,zero,0);
                        }
                    }
                }
...
int32_t vaporum_connectblock(bool fJustCheck, CBlockIndex *pindex,CBlock& block)
{
    ...
    uint64_t signedmask,voutmask; char symbol[KOMODO_ASSETCHAIN_MAXLEN],dest[KOMODO_ASSETCHAIN_MAXLEN]; struct vaporum_state *sp;
